...
export class CardAnimationManager {
    positionManager;
    connectionManager;
    allCards = document.querySelectorAll('.page-card, .server-card');
    ...
    _calculateReplacementPositions(hoveredCard, relatedElements, unrelatedCards) {
        const diagramContainer = document.getElementById('diagram-container');
        const containerRect = diagramContainer?.getBoundingClientRect();
        if (!containerRect)
            return {};
    ...
    _animateRelatedCardToReplacement(card, hoveredCard, replacementPositions) {
        const diagramContainer = document.getElementById('diagram-container');
        const containerRect = diagramContainer?.getBoundingClientRect();
        if (!containerRect)
            return;
    ...
    _drawConnectionsForRelatedCards(hoveredCard, relatedElements) {
        const relatedCards = [...relatedElements.pages, ...relatedElements.servers];
    ...
    _drawPageConnections(pageCard, relatedElements) {
    ...
    _drawServerConnections(serverCard) {
